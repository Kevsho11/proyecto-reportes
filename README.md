...
* **¿Qué problema(s) evita (ej. acoplamiento, violación del principio Abierto/Cerrado)?**
    ...
    2.  **Cumple el Principio Abierto/Cerrado:** El sistema está *cerrado a la modificación* (el código cliente no se toca) pero *abierto a la extensión*. Como pide el requerimiento, si en el futuro queremos agregar un `RenderizadorXML`, solo creamos la nueva clase y añadimos un `case` en la Factory. El código cliente existente seguirá funcionando sin ningún cambio.

### Requerimiento 2: La Construcción de los Reportes

* **¿Qué patrón de diseño creacional eligieron?**
    Se eligió el **Patrón Builder**.

* **¿Por qué este patrón es la solución adecuada?**
    El patrón Builder es ideal para construir objetos complejos paso a paso. Permite separar el proceso de construcción del objeto (Builder) de su representación final (Reporte). Esto permite especificar solo los atributos que necesitamos (especialmente los opcionales), mientras forzamos la inclusión de los atributos obligatorios a través del constructor del Builder. Esto ayuda a tener un código mucho más limpio y legible.

* **¿Qué problemas específicos del "constructor" resuelve?**
    1.  **Evita el "Constructor Telescópico":** No se necesita crear múltiples constructores, solo un Builder que se configura fluidamente.
    2.  **Evita múltiples parámetros y nulls:** El cliente no tiene que llamar a un constructor como, por ejemplo, `new Reporte("Mi Título", "Cuerpo...", null, "Mi autor", null, null, null)`. En lugar de eso, solo llama a los métodos de los atributos opcionales que realmente desea configurar, permitiendo una construcción más flexible y clara.
